class CardModel {
  ...
  // 메타
  /// 마지막 수정 시각 문자열. **앱이 새로 만드는 형식은 한 가지뿐**이다 —
  /// `card_edit_screen`의 `MMM dd, yyyy HH:mm:ss GMT±HH:mm`(.memk 원본 호환).
  /// 가져오기로 들어온 값은 원본 파일의 문자열을 **그대로 보존**한다(형식이 달라도
  /// 덮어쓰지 않는다) — 다른 기기·다른 앱이 적은 시각을 우리가 임의로 바꾸면 그쪽
  /// 기록이 사라지기 때문. 이 컬럼은 표시·정렬 어디에도 쓰이지 않으므로 파싱하지 말 것
  /// (감사 X3-03: "세 곳이 서로 다른 형식으로 쓴다"는 지적의 실제 내용은 이 보존 규칙이다).
